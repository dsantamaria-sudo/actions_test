...
def create_food101_dataloaders(
    ...
    if missing:
        raise ValueError(
            f"Classes not found in Food101: {missing}. Available: {full_train.classes}"
        )
    ...
    train_indices = [
        i for i, label in enumerate(full_train._labels) if label in old_to_new
    ]
    test_indices = [
        i for i, label in enumerate(full_test._labels) if label in old_to_new
    ]
